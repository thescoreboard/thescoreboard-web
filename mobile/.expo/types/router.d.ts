...
declare module 'expo-router' {
  export namespace ExpoRouter {
    export interface __routes<T extends string = string> extends Record<string, unknown> {
      StaticRoutes: `/` | `/(auth)` | `/(auth)/login` | `/(auth)/register` | `/(tabs)` | `/(tabs)/` | `/(tabs)/explore` | `/(tabs)/organiser` | `/(tabs)/profile` | `/_sitemap` | `/explore` | `/login` | `/onboarding` | `/organiser` | `/organiser/create` | `/organiser/score` | `/organiser/tournament/[id]/` | `/profile` | `/register`;
